...
<template>
  <div class="max-container">
    <div class="header">
      <a class="logo" href="#">
        <svg class="arco-icon" viewBox="0 0 48 48" fill="currentColor">
          <path data-v-2ee6cb6b="" fill-rule="evenodd" clip-rule="evenodd"
              ...
              <div class="detail-item-list">
                ...
                <div class="detail-item">
                  ...
                  <div class="value">{{ (item.State.MemUsed / item.Host.MemTotal * 100).toFixed(2) + '%' }}
                    ({{ formatBytes(item.State.MemUsed) }} / {{ formatBytes(item.Host.MemTotal) }})</div>
                </div>
                <div class="detail-item">
                ...
                <div class="detail-item" v-if="hostInfo[item.Host.Name] && hostInfo[item.Host.Name].buy_url">
                  ...
                  <div class="value">
                    <a style="color: #0077ff" :href="hostInfo[item.Host.Name].buy_url" target="_blank"
                      @click.stop="() => { }">{{ hostInfo[item.Host.Name].buy_url }}</a>
                  </div>
                </div>
              </div>
            </a-col>
    ...
    <a-modal v-model:visible="editVisible" :footer="false" :hide-title="true" width="360px">
      ...
      <div class="akile-modal-content">
        <a-date-picker v-model="duetime" :placeholder="$t('due-time-placeholder')"
    ...
    <div class="footer" style="margin-top: 30px">{{ $t('open-source') }} <a
        href="https://github.com/akile-network/akile_monitor">GitHub v0.0.3</a></div>
...
body::before {
  ...
  height: 100%;
  z-index: -1;
  background: url(//cf-api.nianlei.icu/background) center/cover no-repeat;
  filter: blur(0px);
}
